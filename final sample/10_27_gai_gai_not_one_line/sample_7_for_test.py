...
def find_word(filename, word):
    ...
    def find_word_horizontally(grid, word):
        ...
        for i in range (len(grid)):
            for j in range (len(grid[0])- len_word + 1):
                ...
                if grid[i][j] == word[0]:
                    ...
                    while grid[a][b] == word[index] and index < len_word :
                        index += 1
    ...
    def find_word_vertically(grid, word):
        for i in range((grid[0])):
            for j in range(len(grid) - len(word) + 1):
                if grid[i][j] == word[0]:
                    index = 1
                    a , b = i, j + 1
                    while index < len(word) and grid[a][b] == word[index] :
                        b += 1
                        index += 1
                    if index == len(word):
                        return i + 1, j + 1
